<template>
  <div id="app">
    <div style="margin:5px auto;">
      <span style="font-weight: lighter;margin-right: 10px;">为 【{{ role.name }}】 分配权限</span>
...
export default {
  ...
  methods:{
    ...
    async save() {
      let self = this;
      self.selected = [];
      await self.getSelected(self.auths).then(function (){
        ...
        Api.post('/role_auth_save',data).then(function (res) {
          if (res.code === 0){
    ...
    loadPage() {
      ...
      Api.post('/role_auth',data).then(function (res){
        if (res.code === 0){
          self.role = res.data.role;
          self.auths = res.data.auths?res.data.auths:[];
